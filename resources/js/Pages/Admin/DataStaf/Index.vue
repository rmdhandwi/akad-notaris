...
const componentProps = computed(() => {
    switch (currentTab.value) {
        ...
        case 'List':
        return {
            dataStaf : props.dataStaf?.map((p, i) => ({ nomor: i + 1, ...p }))
        };

        case 'Form' :
            if(editDataId.value) {
                return {
                    dataStaf : props.dataStaf?.find(data => data.user_id === editDataId.value)
                }
            }
            else {
                return {}
...
const cancelEdit = () =>
{
     confirm.require({
        ...
        accept: () => {
            ...
            switchComponents('List','Daftar Staf')
        }
    })
}
...
watch(() => editDataId.value, () => {
    if(editDataId.value) {
        switchComponents('Form', 'Edit Staf')
    }
})
</script>
    ...
    <AuthenticatedLayout :pageTitle="pageTitle">
        <template #pageContent>
            ...
            <div class="flex gap-x-4">
                <Button @click="switchComponents('List','Daftar Staf')" label="Daftar Staf" :severity="currentTab==='List'?'primary':'secondary'" icon="pi pi-list" v-if="currentTab==='List'"/>
                <Button @click="cancelEdit()" label="Kembali" severity="secondary" icon="pi pi-arrow-left" v-else/>
                <Button @click="switchComponents('Form','Tambah Staf')" label="Form Staf" :severity="currentTab==='List'?'secondary':'primary'" icon="pi pi-plus"/>
            </div>
            <!-- components -->
